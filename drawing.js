...
function plotNonContactLagrangeMultipliers(STATE_STORY, ParentDivId, width = 500, height = 500){
    // the lagrange multipliers array will change for each state depending on the active contacts
    // so first, we'll plot the lagrange multipliers of the non-evolving constraints ( non contact mainly)
    // the order of constraints is [distance,pin,contact] so for each state we iter over the constraints_distance and get the lagrange multipliers
    //with the corresponding index, we do the same for the constraints_pin with the shift of the index
    //additionally for the name of the traces, for the distance constraints, we'll use the indices of the particles ( first and second indices)
    //for instance constraint_distance_(1,4) for the pin, we use the index of the particle ( first entry of the constraint_pin)
  //all l.multipliers will be plot in the same plot

    let parentDiv = document.getElementById(ParentDivId);
    let tarray = STATE_STORY.map(state => state.time);
    let nconstraintsDistance = STATE_STORY[0].constraints_distance.length;
    let nconstraintsPin = STATE_STORY[0].constraints_pin.length;
    let div_plot_id = ParentDivId + "-lagrange_multipliers";
    let div = parentDiv.querySelector("#" + div_plot_id);
    if (div==null){
        div = document.createElement("div");
        div.id = div_plot_id;
        parentDiv.appendChild(div);
    ...
    let traces = [];
    let layout = {
        title: "Lagrange Multipliers distance and pin constraints",
    ...
    let lagrange_multipliers = STATE_STORY.map(state => state.lagrange_multipliers);

    for (let i = 0; i < nconstraintsDistance; i++){
        let name = "constraint_distance_" + STATE_STORY[0].constraints_distance[i].slice(0,2);
        let x = tarray;
        let y = lagrange_multipliers.map(lm => lm[i]);
        let trace = {
            x: x,
            y: y,
            mode: "lines",
    ...
    for (let i = 0; i < nconstraintsPin; i++){
        let name = "constraint_pin_" + STATE_STORY[0].constraints_pin[i][0];
        let x = tarray;
        let y = lagrange_multipliers.map(lm => lm[i + nconstraintsDistance]);
        let trace = {
    ...
    Plotly.newPlot(div_plot_id, traces, layout);

}

function plotContactLagrangeMultipliers( STATE_STORY, ParentDivId, width = 500, height = 500){
  // the lagrange multipliers array will change for each state depending on the active contacts
  // to build the data to plot, we do the following, we first create a dict with keys being the unique collision ids
  // found in all stories, each collision is of the form [iP,jE,Pidx] where iP is the index of the particle, jE is the index of the edge Pidx is the
  //polygon index, we'll use the string representation of the collision as the key "iP_jE_Pidx"
  //after we've the list of unique ids, we iterate again over the state story and if the collision is present at that state 
  //we add the correspoinding lagrange multiplier, (same index in lagrange_multipliers_contact)
  // if the collision is not present, we add a 0

  let parentDiv = document.getElementById(ParentDivId);
  let tarray = STATE_STORY.map(state => state.time);
  let div_plot_id = ParentDivId + "-lagrange_multipliers_contact";
  let div = parentDiv.querySelector("#" + div_plot_id);
  if (div==null){
  ...
  let collisions = STATE_STORY.map(state => state.collisions);
  let unique_collisions = [];
  for (let i = 0; i < collisions.length; i++){

      for (let collision of collisions[i]){
          let key = `${collision[0]}_${collision[1]}_${collision[2]}`;
          if (!unique_collisions.includes(key)){
              unique_collisions.push(key);
          }
      }
  }
  ...
  let traces = [];
  let layout = {
      title: "Lagrange Multipliers contact constraints",
      xaxis: {
          title: "time",
      },
      yaxis: {
  ...
  for (let state of STATE_STORY){
      let lagrange_multipliers_contact = state.lagrange_multipliers_contact;
      let collisions = state.collisions;
      let x = tarray;
      for (let collision of unique_collisions){
          let idx = unique_collisions.indexOf(collision);
          let lagrange_multiplier = 0;
          for (let i = 0; i < collisions.length; i++){
              let key = `${collisions[i][0]}_${collisions[i][1]}_${collisions[i][2]}`;
              if (key === collision){
                  lagrange_multiplier = lagrange_multipliers_contact[i];
                  break;
              }
          }
          if (traces.length <= idx){
              ...
              traces[idx].y.push(lagrange_multiplier);
          }
      }
  }
...
function plotAngleDistanceConstraints(STATE_STORY, ParentDivId, width = 500, height = 500){
  // Here we'll plot the angle of a rod/distance constraint with the horizontal [1,0] vector
  // we get the angle from the tangent of the rvector = (x2-x1,y2-y1) atan(rvector[1]/rvector[0])
  // we transform the  angle to degrees
// we have to iter over the constraints distance, we get the idxs of the particles, we get the x,y of the particles from STATE.xs
// we calculate the angle and add it to the traces
//for the name of the traces, we'll use the indices of the particles ( first and second indices)
  let parentDiv = document.getElementById(ParentDivId);
  let tarray = STATE_STORY.map(state => state.time);
  let div_plot_id = ParentDivId + "-angle_distance_constraints";
  let div = parentDiv.querySelector("#" + div_plot_id);
  if (div==null){
  ...
  let layout = {
      ...
      title: "Angle Distance Constraints",
      xaxis: {
          title: "time",
      },
      yaxis: {
  ...
  for (let state of STATE_STORY){
      ...
      let xs = state.xs;  
      let x = tarray;
      for (let constraint of constraints_distance){

          let idx1 = constraint[0];
          let idx2 = constraint[1];
          let x1 = xs[idx1][0];
          let y1 = xs[idx1][1];
          let x2 = xs[idx2][0];
          let y2 = xs[idx2][1];
          let rvector = [x2-x1,y2-y1];
          let angle = Math.atan2(rvector[1],rvector[0])*180/Math.PI;
          
          let name = `constraint_distance_${idx1}_${idx2}`;
          let trace = {
